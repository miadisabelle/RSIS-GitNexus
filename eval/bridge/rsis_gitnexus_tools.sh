...
#
# Usage:
#   rsis-gitnexus-query "how does authentication work"
#   rsis-gitnexus-context "validateUser"
#   rsis-gitnexus-impact "AuthService" upstream
#   rsis-gitnexus-cypher "MATCH (n:Function) RETURN n.name LIMIT 10"
#   rsis-gitnexus-overview
...
rsis-gitnexus-query() {
    ...
    if [ -z "$query" ]; then
        echo "Usage: rsis-gitnexus-query <query> [task_context] [goal]"
        echo "Search the code knowledge graph for execution flows related to a concept."
        echo ""
        echo "Examples:"
        echo '  rsis-gitnexus-query "authentication flow"'
        echo '  rsis-gitnexus-query "database connection" "fixing connection pool leak"'
        return 1
    fi
    ...
    _rsis-gitnexus_call query "$args"
}

rsis-gitnexus-context() {
    local name="$1"
    local file_path="${2:-}"

    if [ -z "$name" ]; then
        echo "Usage: rsis-gitnexus-context <symbol_name> [file_path]"
        echo "Get a 360-degree view of a code symbol: callers, callees, processes, file location."
        echo ""
        echo "Examples:"
        echo '  rsis-gitnexus-context "validateUser"'
        echo '  rsis-gitnexus-context "AuthService" "src/auth/service.py"'
        return 1
    fi
    ...
    _rsis-gitnexus_call context "$args"
}

rsis-gitnexus-impact() {
    local target="$1"
    local direction="${2:-upstream}"

    if [ -z "$target" ]; then
        echo "Usage: rsis-gitnexus-impact <symbol_name> [upstream|downstream]"
        ...
        echo ""
        echo "Examples:"
        echo '  rsis-gitnexus-impact "AuthService" upstream'
        echo '  rsis-gitnexus-impact "validateUser" downstream'
        return 1
    fi

    _rsis-gitnexus_call impact "{\"target\": \"$target\", \"direction\": \"$direction\"}"
}

rsis-gitnexus-cypher() {
    local query="$1"

    if [ -z "$query" ]; then
        echo "Usage: rsis-gitnexus-cypher <cypher_query>"
        ...
        echo ""
        echo "Examples:"
        echo "  rsis-gitnexus-cypher 'MATCH (a)-[:CodeRelation {type: \"CALLS\"}]->(b:Function {name: \"save\"}) RETURN a.name, a.filePath'"
        echo "  rsis-gitnexus-cypher 'MATCH (n:Class) RETURN n.name, n.filePath LIMIT 20'"
        return 1
    fi

    _rsis-gitnexus_call cypher "{\"query\": \"$query\"}"
}

rsis-gitnexus-overview() {
    echo "=== Code Knowledge Graph Overview ==="
    _rsis-gitnexus_call list_repos '{}'
}

# Export functions so they're available in subshells
export -f _rsis-gitnexus_call 2>/dev/null
export -f rsis-gitnexus-query 2>/dev/null
export -f rsis-gitnexus-context 2>/dev/null
export -f rsis-gitnexus-impact 2>/dev/null
export -f rsis-gitnexus-cypher 2>/dev/null
export -f rsis-gitnexus-overview 2>/dev/null
